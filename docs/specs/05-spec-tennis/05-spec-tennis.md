...
6. **Scope discipline:** the only files outside `lib/espn/`, `lib/home/`, `lib/sports/`, `lib/favorites/`, `lib/sport-allowlist*`, `components/tournament-card*`, `components/match-card*`, `README.md`, and the spec's own `docs/specs/05-spec-tennis/` directory that this spec modifies are: the homepage route file (`app/(app)/home/page.tsx` or its existing aggregator wiring) to read `activeTennisTournaments` from the aggregator output. No other route handlers, no bottom-nav, no auth, no DB schema changes.
